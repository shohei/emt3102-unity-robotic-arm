...
namespace InverseKinematics {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                if (err_norm < 1E-3) {
                    for (int ii = 0; ii < joint.Length; ii++) {
                        if (angle[i] < minAngle[ii] || angle[ii] > maxAngle[ii]) {
                            outOfLimit = true;
                            break;
                        }
                    }
    ...
    DenseMatrix CalcErr () {
        ...
        var err = DenseMatrix.OfArray (new float[, ] { { perr.x }, { perr.y }, { perr.z }, { rerrVal.x * Mathf.Deg2Rad }, { rerrVal.y * Mathf.Deg2Rad }, { rerrVal.z * Mathf.Deg2Rad }
        });
    ...
    DenseMatrix CalcJacobian () {
        ...
        var J = DenseMatrix.OfArray (new float[, ] { { p0.x, p1.x, p2.x, p3.x, p4.x, p5.x }, { p0.y, p1.y, p2.y, p3.y, p4.y, p5.y }, { p0.z, p1.z, p2.z, p3.z, p4.z, p5.z }, { w0.x, w1.x, w2.x, w3.x, w4.x, w5.x }, { w0.y, w1.y, w2.y, w3.y, w4.y, w5.y }, { w0.z, w1.z, w2.z, w3.z, w4.z, w5.z }
        });
